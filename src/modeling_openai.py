...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, head_mask=None, attn_bias=None):
        w = torch.matmul(q, k)
        if self.scale:
    ...
    def forward(self, x, head_mask=None, attn_bias=None):
        x = self.c_attn(x)
        ...
        a = self._attn(query, key, value, head_mask, attn_bias)
...
class Block(nn.Module):
    ...
    def forward(self, x, head_mask=None, attn_bias=None):
        a = self.attn(x, head_mask=head_mask, attn_bias=attn_bias)
        if self.output_attentions:
            attentions, a = a
        n = self.ln_1(x + a)
        m = self.mlp(n)
...
class OpenAIGPTClsHead(nn.Module):
    ...
    def __init__(self, config, num_labels):
        ...
        self.dropout = nn.Dropout(config.resid_pdrop)  # To reproduce the noise_shape parameter of TF implementation
        self.linear = nn.Linear(config.n_embd, num_labels)
    ...
    def forward(self, hidden_states, token_ids):
        ...
        # mc_token_ids (bsz,)
        multiple_choice_h = torch.stack([
            hidden_states[i, token_ids[i]]
...
class OpenAIGPTModel(OpenAIGPTPreTrainedModel):
    ...
    def forward(self, input_ids, position_ids=None, token_type_ids=None, head_mask=None, attn_bias=None):
        ...
        for i, block in enumerate(self.h):
            outputs = block(hidden_states, head_mask[i], attn_bias=attn_bias)
            if self.output_attentions:
...
class OpenAIGPTDoubleHeadsClsModel(OpenAIGPTPreTrainedModel):
    """OpenAI GPT model with a Language Modeling and a Multiple Choice head ("Improving Language Understanding by Generative Pre-Training").
    ...
    Inputs:
        `input_ids`: a torch.LongTensor of shape [batch_size, sequence_length] with the BPE token
            indices selected in the range [0, total_tokens_embeddings[
        `token_ids`: a torch.LongTensor of shape [batch_size] with the index of the token from
            which we should take the hidden state to feed the classifier (usually last token of the sequence)
        `position_ids`: an optional torch.LongTensor with the same shape as input_ids
            with the position indices (selected in the range [0, config.n_positions - 1[.
        `token_type_ids`: an optional torch.LongTensor with the same shape as input_ids
            You can use it to add a third type of embedding to each input token in the sequence
        ...
        `lm_labels`: optional language modeling labels: torch.LongTensor of shape [batch_size, sequence_length]
            with indices selected in [-1, 0, ..., total_tokens_embeddings]. All labels set to -1 are ignored (masked), the loss
            is only computed for the labels set in [0, ..., total_tokens_embeddings]
        `labels`: optional labels: torch.LongTensor of shape [batch_size]
            with indices selected in [0, ..., num_labels].
        `head_mask`: an optional torch.Tensor of shape [num_heads] or [num_layers, num_heads] with indices between 0 and 1.
            It's a mask to be used to nullify some heads of the transformer. 1.0 => head is fully masked, 0.0 => head is not masked.

    Outputs:
        if `lm_labels` and `labels` are not `None`:
            Outputs a tuple of losses with the language modeling loss and the classification loss.
        else: a tuple with
            `lm_logits`: the language modeling logits as a torch.FloatTensor of size [batch_size, sequence_length, total_tokens_embeddings]
            `cls_logits`: the classification logits as a torch.FloatTensor of size [batch_size, num_labels]

    Example usage:
    ```python
    # Already been converted into BPE token ids
    input_ids = torch.LongTensor([[31, 51, 99], [15, 5, 0]])  # (bsz, seq length)
    token_ids = torch.LongTensor([2, 1]) # (bsz,)

    config = modeling_openai.OpenAIGPTConfig()

    model = modeling_openai.OpenAIGPTDoubleHeadsClsModel(config, num_labels=4)
    lm_logits, cls_logits = model(input_ids, token_ids)
    ```
    """

    def __init__(self, config, output_attentions=False, keep_multihead_output=False, num_labels=4):
        super(OpenAIGPTDoubleHeadsClsModel, self).__init__(config)
        self.transformer = OpenAIGPTModel(config, output_attentions=output_attentions,
                                             keep_multihead_output=keep_multihead_output)
        self.lm_head = OpenAIGPTLMHead(self.transformer.tokens_embed.weight, config)
        self.cls_head = OpenAIGPTClsHead(config, num_labels)
        self.apply(self.init_weights)
    ...
    def forward(self, input_ids, token_ids, lm_labels=None, labels=None, token_type_ids=None,
                position_ids=None, head_mask=None, attn_bias=None):
        hidden_states = self.transformer(input_ids, position_ids, token_type_ids, head_mask, attn_bias)
        if self.transformer.output_attentions:
            all_attentions, hidden_states = hidden_states
        hidden_states = hidden_states[-1]

        lm_logits = self.lm_head(hidden_states)
        cls_logits = self.cls_head(hidden_states, token_ids)
        losses = []
        if lm_labels is not None:
            shift_logits = lm_logits[..., :-1, :].contiguous()
            shift_labels = lm_labels[..., 1:].contiguous()
        ...
        if labels is not None:
            # loss_fct = CrossEntropyLoss()
            # losses.append(loss_fct(cls_logits.view(-1, cls_logits.size(-1)), labels.view(-1)))
            loss_fct = BCEWithLogitsLoss()
            losses.append(loss_fct(cls_logits.view(-1, cls_logits.size(-1)), labels))
        if losses:
            return losses
        if self.transformer.output_attentions:
            return all_attentions, lm_logits, cls_logits
